...
def debug_scenario_a():
    ...
    try:
        ...
        # 检查分组名称是否正确
        expected_groups = ["province_score_match", "national_score_match"]
        actual_groups = list(final_result['groups'].keys())
        
        if all(group in actual_groups for group in expected_groups):
            print("✅ 分组结构验证成功: 包含期望的分组")
        else:
            print(f"❌ 分组结构验证失败: 缺少分组 {set(expected_groups) - set(actual_groups)}")
