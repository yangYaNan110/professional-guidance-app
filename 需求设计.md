...
| **对象存储** | MinIO | 9000/9001 | 对象存储 | 1 |
| **消息队列** | Kafka | 9092 | 消息队列 | 1 |
| **代理服务** | Nginx | 80/443 | 反向代理 | 2 |
...
**启动顺序重要性**：
1. **基础设施层**（端口固定）：数据库、缓存、搜索、存储
2. **代理层**（端口固定）：Nginx反向代理
3. **服务层**（端口固定）：API网关、业务服务
4. **前端层**（端口固定）：Web应用、移动应用
...
# 按顺序启动服务
docker-compose -f docker/docker-compose.ports.yml up -d
```
...
```bash
# 启用Redis缓存服务
docker-compose -f docker-compose.simple.yml start redis
