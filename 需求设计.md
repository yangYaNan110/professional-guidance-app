...
| 数据源 | 类型 | 优先级 | 说明 |
|-------|------|--------|------|
| **B站** | 视频+文章 | 1 | 视频热点、科普内容（带播放量、时长） |
| **知乎** | 问答+文章 | 2 | 专业讨论、深度分析（带阅读量） |
| **36氪** | 新闻 | 3 | 科技创投新闻（带阅读量） |
| **虎嗅** | 资讯 | 4 | 科技商业资讯（带阅读量） |
| **微博** | 热搜 | 5 | 社会热点和舆论（带讨论量） |
| **澎湃新闻** | 新闻 | 6 | 权威新闻报道 |
...
def build_hot_event_keywords(major_name: str) -> List[str]:
    """构建热点事件搜索关键词"""
    return [
        f"{major_name}最新消息",      # 最新消息
        f"{major_name}热点",          # 热点事件
        f"{major_name}新闻",          # 新闻动态
        f"{major_name}重大突破",      # 重大突破
        f"{major_name}2024",          # 年度事件
